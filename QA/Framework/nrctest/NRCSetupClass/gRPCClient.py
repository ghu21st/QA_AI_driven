...
class gRPCClient:
    ...
    def __init__(self, gRPC_server=None, gRPCs_port=None, gRPC_port=None):
        ...
        # Global Auth - Get NRC client ID and client secret for NRaaS global Auth token
        if context.config['NRCClientId']:
            self._clientid_ = context.config['NRCClientId']
        else:
            self._clientid_ = ""
        if context.config['NRCClientSecret']:
            self._clientsecret_ = context.config['NRCClientSecret']
        else:
            self._clientsecret_ = ""
        ...
        if context.config['GAGrantType']:
            self._granttype_ = context.config['GAGrantType']
        else:
            self._granttype_ = ""
        if context.config['GAScope']:
            self._scope_ = context.config['GAScope']
        else:
            self._scope_ = ""
        ...
        if not self._nrctest_:
            ...
            # For test without Global Auth - Get certificate for nrc secure connection
            with open('/QA/NRC_Test/Framework/nrctest/NRCSetupClass/cacert.pem', 'rb') as certFile:
                trust_certs = certFile.read()
                print('\nNRaaS certificate content:\n' + str(trust_certs))  # for debug
            certFile.close()

            # check if global auth enabled or bypassed
            if self._globalauthurl_ == "":
                # option 1: for test without Global Auth, add certs to gRPC ssl channel for secure connection
                self._credentials_ = grpc.ssl_channel_credentials(root_certificates=trust_certs)  # for test without Global Auth
            else:
                # option 2: for test with Global Auth
        ...
        # check if for NRC test with secure connect
            ...
            self._secure_url_ = '{}:{}'.format('nrc', self._secure_port_)  ### for NRC secure connection test
            ### self._secure_url_ = '{}:{}'.format(self._hostname_, '443')  ### for NRaaS secure connection test
        ...
        if 'NRCAudioPath' in context.config:
            ...
            self._audio_path_ = '/package/unarchive/Framework/nrctest/audio/'  # default value
    ...
    # ---- Get Global Auth token for NRaaS in CD4
    def get_GAtoken(self):
        # encode client id
        encoded_clientid = urllib.parse.quote(self._clientid_)

        # debug
        msg = '\nGlobal Auth settings: \n'
        msg += 'url: ' + str(self._globalauthurl_) + '\n'
        msg += 'grant type: ' + str(self._granttype_) + '\n'
        msg += 'scope: ' + str(self._scope_) + '\n'
        msg += 'client id: ' + str(self._clientid_) + '\n'
        msg += 'encoded client id: ' + str(encoded_clientid) + '\n'
        msg += 'client secret: ' + str(self._clientsecret_) + '\n'
        if self._logging_ > 0:
            print(msg)
            self._logger_.info(msg)
        ...
        # Define request data
        data = {
            'grant_type': self._granttype_,
            'scope': self._scope_
        }

        # Get response
        response = requests.post(self._globalauthurl_, data=data, auth=(encoded_clientid, self._clientsecret_))
